...
struct EditAlbumItemView: View {
    ...
    func edit() {
        ...
        var editedAlbumItem = AlbumItem(recordId: viewModel.recordId ,name: viewModel.name, artist: viewModel.artist, genre: viewModel.genre, listens: viewModel.listens, lastListened: viewModel.lastListened! )
        if editedAlbumItem.listens > viewModel.listens {
            editedAlbumItem.lastListened = Date()
        }
        Task{
            try await cloudModel.updateAlbum(editedAlbumItem: editedAlbumItem)
        }
        ...
        NavigationView{
            VStack {
                Form {
                    ...
                    // Listens
                    HStack{
                        Picker("Listens", selection: $viewModel.listens) {
...
#Preview {
    EditAlbumItemView(index: 0, color: Color("Cyan"), viewModel: EditAlbumItemViewModel(albumItem: AlbumItem(recordId: CKRecord.ID(recordName: "Album"), name: "Test", artist: "Test", genre: "test", listens: 5, lastListened: Date())), EditAlbumItemPresented: Binding(get: {return true}, set: {_ in})).environmentObject((CloudModel()))
